...
class OrderController extends GetxController implements GetxService {
  ...
  /// Reset the per-payment-flow guard inside the underlying service so
  /// a re-entered payment screen does not inherit a stale "already
  /// handled" state from a previous session with the same controller
  /// instance (GetxService is a long-lived singleton).
  void resetPaymentRedirectGuard() {
    // The service implementation owns the guard state.
    (orderServiceInterface as dynamic).resetPaymentRedirectGuard();
  }
