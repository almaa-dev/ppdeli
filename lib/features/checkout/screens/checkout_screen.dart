...
class CheckoutScreenState extends State<CheckoutScreen> {
  ...
  ///
  /// All checkout screens share the same resolver instance because the
  /// business rule itself is stateless; it only depends on the values
  /// passed in by the caller.
  final DeliveryFeeResolver _deliveryFeeResolver = DeliveryFeeResolver();
  ...
  void initState() {
    ...
    // Schedule the call for the next microtask
    Future.microtask(() {
      initCall();
    });
  }
    ...
    return Scaffold(
      ...
      body: guestCheckoutPermission || AuthHelper.isLoggedIn() ? GetBuilder<CheckoutController>(builder: (checkoutController) {
        ...
        return GetBuilder<CouponController>(builder: (couponController) {
          ...
          // Delegate the final delivery-fee decision to DeliveryFeeResolver.
          //
          // The resolver has three explicit outcomes (loading / free / paid),
          // so the UI can never accidentally treat a "not yet calculated"
          // -1 sentinel as Free. This is the root cause fix for the
          // legacy "Free -> $5 -> Free" flicker.
          final DeliveryFeeSnapshot feeSnapshot = _deliveryFeeResolver.resolve(
            orderType: checkoutController.orderType,
            orderAmount: orderAmount,
            store: checkoutController.store,
            distanceKm: checkoutController.distance,
            originalCharge: originalCharge,
            guestAddress: checkoutController.guestAddress,
            couponController: couponController,
          );

          double deliveryCharge;
          if (feeSnapshot.isLoading) {
            // Keep -1.0 as the in-band "calculating" sentinel that the
            // bottom_section widget already understands. The UI shows
            // "calculating" / "—" instead of "Free" until the snapshot
            // becomes free or paid.
            deliveryCharge = -1.0;
          } else {
          ...
          // build. We rebuild only after a state change, never between
          // loading and a calculated value.
          String nextDeliveryChargeForView;
          if (checkoutController.orderType == 'take_away') {
            nextDeliveryChargeForView = 'free'.tr;
          } else if (feeSnapshot.isLoading) {
            nextDeliveryChargeForView = 'calculating'.tr;
          } else if (feeSnapshot.isFree) {
            nextDeliveryChargeForView = 'free'.tr;
          } else {
            nextDeliveryChargeForView = PriceConverter.convertPrice(deliveryCharge);
          }
          if (nextDeliveryChargeForView != _deliveryChargeForView) {
            // State variable is local to this widget. Schedule a rebuild
            // AFTER the current frame so TopSection / DeliveryOptionButton
            // pick up the new value on the next rebuild triggered by
            // CheckoutController.update().
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (!mounted) return;
              setState(() {
                _deliveryChargeForView = nextDeliveryChargeForView;
              });
            });
          }
  ...
  double _calculateOriginalDeliveryCharge({required Store? store, required AddressModel address, required double? distance, required double? extraCharge, double? surgePrice, String? surgePriceType}) {
    ...
    if(store != null && distance != null) {
      ...
      }else if(maximumCharge > 0 && deliveryCharge > maximumCharge) {
        if (kDebugMode) {
  ...
  double _calculateTotal({
    ...
    // deliveryCharge == -1 is the in-band "still calculating" sentinel
    // coming out of DeliveryFeeResolver. We must not add -1 to the total
    // (it would produce a negative value while the async distance / extra
    // charge are still being fetched).
    final double effectiveDeliveryCharge = deliveryCharge < 0 ? 0.0 : deliveryCharge;

    return PriceConverter.toFixed(
        subTotal + effectiveDeliveryCharge - discount- couponDiscount + (taxIncluded ? 0 : tax)
