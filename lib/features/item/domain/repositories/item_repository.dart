...
class ItemRepository implements ItemRepositoryInterface {
  ...
  /// Fetch a single product's full details from the network.
  ///
  /// IMPORTANT: We deliberately do NOT use cache here. Caching the details
  /// endpoint is the responsibility of the new Enterprise Product Cache
  /// System (see `ProductCacheHelper`). The repository only ever talks to
  /// the network; the controller layer orchestrates the SWR read order.
  ///
  /// As a side effect we persist the RAW response body into the cache
  /// helper so subsequent reads are instant. We save `response.body` (the
  /// full JSON map) — NOT `item.toJson()` — to guarantee no field is lost.
  Future<Item?> _getItemDetails(int? itemID, bool isCampaign) async {
    ...
    if (response.statusCode == 200) {
      // Persist the full raw JSON to the enterprise cache so the next open
      // of the same product is instant and offline-aware.
      try {
        if (response.body is Map<String, dynamic>) {
          await ProductCacheHelper.save(
            itemId: itemID!,
            data: Map<String, dynamic>.from(response.body as Map),
          );
        }
      } catch (_) {/* best-effort, never break the network path */}
