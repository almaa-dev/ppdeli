...
import 'package:pickles_and_pies/common/widgets/quantity_button.dart';
...
class _CartItemWidgetState extends State<CartItemWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    final bool showUnitType = splashController.configModel!.moduleConfig!.module!.unit!
        && widget.cart.item!.unitType != null
        && !splashController.getModuleConfig(widget.cart.item!.moduleType).newVariation!;
    final bool shouldShowFoodType = AppConstants.showFoodType
                  ...
                  Expanded(
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
                      Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
                        ...
                        SizedBox(width: (showUnitType || shouldShowFoodType) ? Dimensions.paddingSizeExtraSmall : 0),

                        if (showUnitType || shouldShowFoodType)
                          Padding(
                            padding: const EdgeInsets.only(bottom: 3),
                            child: showUnitType ? Container(
                      ...
                      // Display item-level note under the product name (only if note is non-empty)
                      (widget.cart.note != null && widget.cart.note!.trim().isNotEmpty) ? Padding(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            borderRadius: BorderRadius.circular(Dimensions.radiusSmall),
                            border: Border.all(color: Theme.of(context).primaryColor.withValues(alpha: 0.25), width: 0.5),
                          ),
                          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Icon(Icons.sticky_note_2_outlined, size: 14, color: Theme.of(context).primaryColor),
  ...
  double? _calculatePrice({required Item? item, bool isStartingPrice = true}) {
    ...
    if(item.variations!.isNotEmpty && !newVariation) {
      // Legacy variation module: variation.price already INCLUDES the base
      // item.price (the server stores it as a single combined amount), so
      // the starting/ending price is simply the cheapest / most expensive
      // variation. No need to fold item.price in again.
      ...
      // Food / new variation module: item.price is the base price, and the
      // user-selected variation options add on top via optionPrice. We have
      // to fold those chosen options into the displayed price so that the
      // cart line shows the TOTAL (item + chosen variation), matching the
      // merged "Item Price" line in the pricing summary.
      double variationAddon = _selectedFoodVariationOptionTotal(item: item);
      startingPrice = (item.price ?? 0) + variationAddon;
    }
    if(isStartingPrice) {
  ...
  /// Sum of the `optionPrice` for every food-variation value the user has
  /// currently ticked in the cart for this item. Returns 0 for legacy
  /// variation items or when no food variation is selected yet.
  double _selectedFoodVariationOptionTotal({required Item? item}) {
    double total = 0;
    if (item?.foodVariations == null || widget.cart.foodVariations == null) {
      return total;
    }
    for (int groupIndex = 0;
        groupIndex < item!.foodVariations!.length;
        groupIndex++) {
      final group = item.foodVariations![groupIndex];
      if (group.variationValues == null) continue;
      if (groupIndex >= widget.cart.foodVariations!.length) break;
      final selected = widget.cart.foodVariations![groupIndex];
      for (int i = 0; i < group.variationValues!.length; i++) {
        if (i >= selected.length) break;
        if (selected[i] == true) {
          total += group.variationValues![i].optionPrice ?? 0;
        }
      }
    }
    return total;
