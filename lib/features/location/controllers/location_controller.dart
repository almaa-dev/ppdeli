...
import 'package:pickles_and_pies/common/widgets/no_internet_screen.dart';
...
import 'package:pickles_and_pies/features/location/domain/models/prediction_model.dart';
...
import 'package:pickles_and_pies/features/location/widgets/module_dialog_widget.dart';
...
class LocationController extends GetxController implements GetxService {
  ...
  /// Seed [pickPosition] with an initial coordinate so the "pick location"
  /// button is enabled as soon as the map is shown — without this the
  /// first tap on the button was a no-op because [pickPosition] was still at
  /// the (0, 0) default until the camera moved.
  void seedPickPosition({required double latitude, required double longitude}) {
  ...
  Future<AddressModel> setLocation(String? placeID, String? address, GoogleMapController? mapController) async {
    ...
    // Refresh zone/button state so the "Pick location" button is enabled
    // immediately after the user picks a result from the search autocomplete.
    ...
    if (zoneResponse.isSuccess) {
      _zoneID = zoneResponse.zoneIds.isNotEmpty ? zoneResponse.zoneIds[0] : 0;
    }
